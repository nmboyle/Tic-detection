...
def cal_tp_fp_fn_by_video(test_video_names,
                        detections,all_test_gt_boxes_names,GLOBAL_test_gt_boxes,
                        sapa=None,experiment_name=None,write_out_results=False,
                        CLASSES=None, CAL_MAP_TABLE=None,
                        iou_threshold = 0.5, logger=None):
    ...
    for k,v in all_det.items():
        ...
        if True:
            # print('v',v)
            # print('cur_v_gt_boxes', cur_v_gt_boxes)
            # print('cur_v', cur_v)
            ...
            pred_boxes_with_score = torch.cat(v) # (..., 3)
            print('before nms',pred_boxes_with_score.shape)
            if len(pred_boxes_with_score) > 1:
                pred_boxes,score = pred_boxes_with_score[:,:-1], pred_boxes_with_score[:,-1].reshape(-1,1)
                idx, count = nms(pred_boxes, score, 
                                nms_threshold=config['eval_nms_threshold'], top_k=config['eval_nms_topk'], use_EIoU=config['eval_nms_use_EIoU'])
                # print(count)
                ...
                print('only one box, no need to nms')
                results = pred_boxes_with_score
        # except:
        #     if logger is None:
        #         print(f'this session {k} has no det! moving on.............')
        #     else:
        #         logger.info(f'this session {k} has no det! moving on.............')
        #     continue
        ...
        all_recall_precision = torch.cat((tp_id,fp_id,results[:,-1].reshape(-1,1)), dim=1)
        
        all_all_recall_precision.append(all_recall_precision)
        ...
        num_tp, num_fp = tp[-1], fp[-1]
        num_fn = cur_v_gt_boxes.shape[0] - num_tp

        recall = tp / cur_v_gt_boxes.shape[0]
        precision = tp / np.maximum(tp + fp, np.finfo(np.float64).eps)
        ...
        if logger is None:
            print(f'{k} AP:{ap} | TP:{int(num_tp)} | FP:{int(num_fp)} | FN:{int(num_fn)}')
        else:
            logger.info(f'{k} AP:{ap} | TP:{int(num_tp)} | FP:{int(num_fp)} | FN:{int(num_fn)}')
        print('\n')

    if len(all_all_recall_precision) == 0:
        all_recall_precision = []
        num_tp, num_fp = 0,0
        num_fn = 0
        for v in FN.values():
        ...
        mrec, mpre, ap, auc = 0,0,0,0

        print('No detections at all! AP = 0')
    else:
        all_recall_precision = torch.cat(all_all_recall_precision)
        idx = all_recall_precision[:,-1].sort(descending=True)[1]
        all_recall_precision = all_recall_precision[idx]
        all_recall_precision = all_recall_precision.numpy()
        tp = np.cumsum(all_recall_precision[:,0])
        ...
        auc = np.trapz(mpre,mrec)

        print('AP for Class Tic: {} (trapz: {})'.format(ap, auc))


    return mrec, mpre, ap, auc, \
                TP, MatchedGT, FP, FN, num_tp, num_fp, num_fn, \
                TP_names, MatchedGT_names, FP_names, FN_names, \
                all_recall_precision
